(function () {
  ...
  const STORAGE_LOCAL_USERS = "tqa_local_users"; // Mismo nombre que en auth.js
  ...
  // Obtiene el ID que auth.js inyectó en la URL
  ...
  function loadDataByClientId(id) {
    // 1. Intentar cargar usuario registrado localmente (auth.js)
    try {
  ...
  function renderTable(ops) {
    ...
    tbody.innerHTML = "";
    if (!ops || ops.length === 0) {
        tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; padding: 20px; color: #666;">No existen operaciones recientes.</td></tr>`;
        return;
    }
  ...
  if (btnVerTodo) {
      // Redirigir a cartera unificada, pasando el ID del cliente para (futuro) filtrado
      btnVerTodo.addEventListener("click", () => window.location.href = `cartera.html?id=${clientId}`);
  }
